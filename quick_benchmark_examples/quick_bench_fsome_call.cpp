...
struct fsome { SBO sbo {0}; bool copy {true}; bool move {true}; bool empty_state {true}; bool check_empty {VX_HARDENED}; };
}// namespace cfg
/// ===== [ FWD Declarations ] =====
...
static void call_fsome_no_sbo(benchmark::State& state) {
    using vx::fsome;
    std::mt19937 mt{};
    fsome<Shape, vx::cfg::fsome{.sbo{0}}> shape {};
    unsigned long sides {0};
    for (auto _ : state) {
...
static void call_fsome_sbo(benchmark::State& state) {
    using vx::fsome;
    std::mt19937 mt{};
    fsome<Shape, vx::cfg::fsome{.sbo{24}}> shape {};
    unsigned long sides {0};
    for (auto _ : state) {
        if (mt() % 2 == 0) { shape = Circle{}; } 
        else { shape = Square{}; }
...
BENCHMARK(call_fsome_sbo);
BENCHMARK(call_fsome_no_sbo);